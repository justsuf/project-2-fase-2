document.addEventListener("DOMContentLoaded", () => {
  ...
  function hidePopupOnScrollOrDrag() {
    popup.classList.remove('visible');
    vergrootglazen.forEach(v => {
      if (v.dataset.link === currentLink) {
  ...
  function update() {
    ...
    vergrootglazen.forEach(v => v.style.transform = `scale(${1 / scale})`);
    ticking = false;
  }
  ...
  popupButton.addEventListener('click', () => { if (currentLink) window.location.href = currentLink; });
  popupClose.addEventListener('click', () => popup.classList.remove('visible'));
  document.addEventListener('click', (e) => { if (!popup.contains(e.target)) popup.classList.remove('visible'); });
});